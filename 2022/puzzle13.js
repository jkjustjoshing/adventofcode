...
function get(arr, i) {
  if (typeof arr === "number") {
    return i === 0 ? arr : undefined;
  } else {
    return arr[i];
  }
}
function compare(a, b, level = 0) {
  log(level, "Starting comparison:");
  log(level + 1, a);
  log(level + 1, b);
  if (typeof a === "number" && typeof b === "number") {
    log(level + 1, `Comparing numbers ${a} and ${b}`, b - a);
    return b - a;
  }

  const length = Math.max(len(a), len(b));
  for (let i = 0; i < length; i++) {
    if (i >= len(a)) {
      log(level + 1, `Comparing arrs - ${a} shorter than ${b}`, 1);
      return 1;
    }
    if (i >= len(b)) {
      log(
        level + 1,
        `Comparing arrs - ${a} longer than ${b}, ${i}, ${len(b)}`,
        -1
      );
      return -1;
    }

    log(level + 1, "Recursing");
    const comp = compare(get(a, i), get(b, i), level + 1);
    log(level + 1, "Exit recursing");
    log(level + 1, "comp=", comp);
    if (comp !== 0) {
      return comp;
    }
  }
...
console.log(
  `Part 1: ` +
    lines
      .map(({ one, two }, i) => (compare(one, two) > 0 ? i + 1 : 0))
      .reduce((a, b) => a + b)
);
const arr = lines.flatMap((l) => [l.one, l.two]);
arr.push([[2]]);
arr.push([[6]]);
arr.sort(compare).reverse();
const ind1 =
  1 +
  arr.findIndex((e) => e.length === 1 && e[0].length === 1 && e[0][0] === 2);
const ind2 =
  1 +
  arr.findIndex((e) => e.length === 1 && e[0].length === 1 && e[0][0] === 6);
console.log("Part 2: " + ind1 * ind2);
